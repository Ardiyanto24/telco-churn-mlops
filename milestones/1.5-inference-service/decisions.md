...
### 9. (Turunan, ditemukan saat eksekusi Checkpoint 2) Tambah `lightgbm`+`xgboost` sebagai dependency inti

**Keputusan:** Tambahkan `lightgbm==4.7.0` dan `xgboost==3.4.0` ke `dependencies` inti `pyproject.toml`, PROVISIONAL (pola sama pandas/numpy M1.2 -- dikunci ke versi yang terbukti jalan di environment ini, bukan versi training asli DS yang tidak tercatat).

**Kenapa:** Bukan keputusan baru yang perlu ditanyakan ke user — dipaksa oleh kenyataan konkret: `joblib.load('artifacs/model/model_final.joblib')` gagal dengan `ModuleNotFoundError: No module named 'lightgbm'` saat pertama kali dicoba (Checkpoint 2, `test_pyfunc_model.py`). `model_final.joblib` ternyata `VotingClassifier` berisi `LGBMClassifier` + 2x `XGBClassifier` (`named_estimators_`: `lightgbm_class_weight`, `xgboost_class_weight`, `xgboost_smote`) — konsisten dengan `tccp-modeling-baseline-v2.ipynb`/`tccp-hyperparameter-tuning.ipynb` yang memang memakai XGBoost+LightGBM+RandomForest (`notebook-audit.md` Bagian E). Gap ini SUDAH diketahui sejak M1.1 audit (`notebook-audit.md` Bagian F, baris "xgboost"/"lightgbm": "Tidak ditemukan" versi) dan dicatat di KT-3 (`docs/keputusan-tertunda.md`) — tapi belum pernah benar-benar BLOCKING sampai sekarang, karena M1.2-1.4 hanya pernah memuat `preprocessor.joblib`, tidak pernah `model_final.joblib`. M1.5 Checkpoint 2 adalah titik pertama model sungguhan dimuat+dipanggil. Setelah instalasi, `joblib.load()` berhasil dan `pandas`/`numpy`/`scikit-learn` tetap persis di versi M1.2 (tidak terdowngrade).

### 10. (Turunan, ditemukan saat verifikasi KK1 Checkpoint 5) Tambah `sqlalchemy`+`alembic` sebagai dependency eksplisit

**Keputusan:** Tambahkan `sqlalchemy==2.0.52` dan `alembic==1.19.1` ke `dependencies` inti `pyproject.toml`.

**Kenapa:** Bukan keputusan baru — dipaksa oleh kegagalan konkret saat verifikasi KK1 (instalasi dari venv terpisah, benar-benar bersih). `predictor.predict()` gagal `UnsupportedModelRegistryStoreURIException` untuk skema `sqlite:///` (backend registry lokal, Keputusan #1/#7) di venv yang HANYA berisi dependency yang dideklarasikan `pyproject.toml`. Akar masalah: `mlflow-skinny` (Keputusan #8) tidak mendeklarasikan `sqlalchemy`/`alembic` sebagai dependency-nya sendiri, padahal keduanya WAJIB ada supaya store registry mlflow bisa resolve skema URI `sqlite://`. Gap ini tidak pernah kelihatan sebelumnya karena venv pengembangan kebetulan masih menyimpan `sqlalchemy`/`alembic` sebagai sisa instalasi `mlflow` PENUH sebelum diganti ke `mlflow-skinny` (Checkpoint 0) -- `pip uninstall mlflow` tidak ikut menghapus dependency transitifnya. Ini justru bukti nyata KENAPA KK1 (uji dari venv benar-benar terpisah) diperlukan sebagai gerbang wajib, bukan langkah opsional -- kesalahan seperti ini tidak akan pernah terlihat kalau hanya diuji dari venv pengembangan yang sudah "kotor" oleh riwayat instalasi sebelumnya. Diverifikasi ulang: venv bersih baru (tanpa instalasi manual apa pun di luar `pip install <repo>`) berhasil memanggil `predict()` end-to-end setelah `pyproject.toml` diperbaiki.
