...
1. `kubectl get pods -n churn-prediction` — cek kolom `READY` (`1/1` vs `0/1`) dan `RESTARTS`. **Kalau ada LEBIH DARI 1 pod** (mis. rolling update sedang berlangsung, atau HPA M3.11 sedang aktif menambah replica): `curl http://localhost/...` HANYA representatif kalau SEMUA pod bermasalah — Service load-balance ke pod yang `Ready` SAJA, jadi curl ke Service akan tetap 200 selama MASIH ADA satu pod sehat, walau ada pod lain yang rusak. Untuk diagnosis pod SPESIFIK (mis. pod baru hasil rollout yang dicurigai bermasalah): `kubectl port-forward pod/<nama-pod> 8001:8000` di terminal terpisah, lalu `curl http://localhost:8001/healthz` (ganti port Service `80` dengan port lokal port-forward `8001`) — baru curl langsung ke pod itu, bukan lewat Service (ditemukan M3.12 Checkpoint 5: curl ke Service tidak pernah menunjukkan kegagalan pod baru selama pod lama masih melayani).
2. `curl http://localhost/healthz` (atau via port-forward pod spesifik, lihat poin 1) — proses HIDUP atau tidak (livenessProbe, restart trigger kalau gagal 3x berturut, `periodSeconds:15`).
3. `curl http://localhost/readyz` (atau via port-forward pod spesifik) — model SIAP dipakai atau tidak (readinessProbe, keluar dari trafik Service kalau gagal, TIDAK restart, `periodSeconds:5`).
4. **Kombinasi gejala menentukan diagnosis — DUA POLA BERBEDA untuk config registry rusak, jangan disamakan:**
   - **Host TIDAK VALID/tidak reachable TAPI resolve DNS berhasil** (mis. Postgres down tapi hostname benar, pola M3.2 KK3): `healthz` 200, `readyz` non-200 — proses SEMPAT membuka port dulu baru gagal readiness. Cek `kubectl logs -n churn-prediction <pod>` untuk retry backoff MLflow (`psycopg2.OperationalError`/`SQLAlchemy engine could not be created`) — normalnya self-heal dalam ~100 detik (M3.2 KK3).
   - **Host GAGAL RESOLVE DNS SAMA SEKALI** (mis. hostname salah ketik/tidak exist, pola M3.11 CP2/M3.12 Checkpoint 5): `healthz` JUGA gagal (`000`/connection refused) — port belum sempat terbuka SAMA SEKALI, retry backoff terjadi SEBELUM Uvicorn listen. Retry backoff EKSPONENSIAL TANPA BATAS (bukan give-up setelah ~100 detik) — kalau dibiarkan, `startupProbe` (budget 300 detik) akan MEMBUNUH pod ini (bukan self-heal, beda dari pola pertama).
