...
- **Bug urutan kolom (Keputusan #10) hanya diuji untuk KASUS yang ditemukan** (dua urutan spesifik: SQL vs pydantic, plus satu urutan `reversed()` tambahan) — TIDAK ada test eksplisit yang memverifikasi properti umum "SEMUA permutasi urutan kolom menghasilkan hasil sama" (mis. lewat `hypothesis`/property-based testing). Fix diyakini benar berdasarkan pemahaman root-cause (reorder eksplisit ke urutan tetap SEBELUM pipeline, bukan bergantung urutan input apa pun), bukan diverifikasi exhaustif untuk semua 19! kemungkinan urutan.

**Update (follow-up selesai, sesi sama):** Test regresi permanen ditambahkan ke `tests/inference/test_pyfunc_model.py` — `test_predict_invariant_to_input_column_order` (urutan `reversed()`) dan `test_predict_matches_regardless_of_request_schema_field_order` (urutan PERSIS `ChurnPredictionRequest.model_dump()` yang memicu bug asli). Dibuktikan lewat bukti negatif eksplisit: baris reorder di `pyfunc_model.py` dinonaktifkan sementara → kedua test GAGAL persis seperti diprediksi → dikembalikan → `pytest tests/ -q` penuh 194/194 lulus.
